...
{
    public class BDSMarriageController : Controller
    {
        private readonly IBDSMarriageService _service;

        public BDSMarriageController(IBDSMarriageService service)
        {
            _service = service;
        }
        ...
        {
            return View(new BDSMarriage { CreateDate = DateTime.Now, CreateUser = 1, ID = 0 });
        }

        [HttpPost]
        public ActionResult Create(BDSMarriage model)
        {
        ...
        {
            BDSMarriage model = _service.GetItem(id);
            return View(model);
        }

        [HttpPost]
        public ActionResult Edit(BDSMarriage model)
        {
            if (!ModelState.IsValid)
            {
        ...
        public JsonResult DeActiveConfirmed
            (int id)
        {

...
{
    public class BDSTypeContactController : Controller
    {
        private readonly IBDSTypeContactService _service;

        public BDSTypeContactController(IBDSTypeContactService service)
        {
            _service = service;
        }
        [DisplayName(@"Type Contact management")]
        ...
        {
            return View(new BDSTypeContact { CreateDate = DateTime.Now, CreateUser = 1, ID = 0 });
        }

        [HttpPost]
        public ActionResult Create(BDSTypeContact model)
        {
        ...
        {
            BDSTypeContact model = _service.GetItem(id);
            return View(model);
        }

        [HttpPost]
        public ActionResult Edit(BDSTypeContact model)
        {
            if (!ModelState.IsValid)
            {

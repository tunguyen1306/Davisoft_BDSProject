...
{
    public class BDSAreaController : Controller
    {
        private readonly IBDSAreaService _service;

        public BDSAreaController(IBDSAreaService service)
        {
            _service = service;
        }
        [DisplayName(@"Area management")]
        ...
        {
            return View(new BDSArea{CreateDate = DateTime.Now,CreateUser = 1,ID = 0});
        }

        [HttpPost]
        public ActionResult Create(BDSArea model)
        {
        ...
        {
            BDSArea model = _service.GetItem(id);
            return View(model);
        }

        [HttpPost]
        public ActionResult Edit(BDSArea model)
        {
            if (!ModelState.IsValid)
            {
